...
1. **Authentication.** `AuthRepository.register` hashes the password (salted PBKDF2 via `PasswordHasher` — current scheme is HMAC-SHA256 @ 600k iterations with a version tag) and writes a `UserEntity` (`user_pass_hash` + `user_pass_salt`), rejecting duplicate emails; `login` reads the user back and `PasswordHasher.verify`s version-aware (legacy HMAC-SHA1/100k hashes still verify and are transparently re-hashed to the current scheme on success), using a constant-time comparison. On success the `LoginViewModel` calls `SessionRepository.signIn(...)` (DataStore). Login's **Skip** enters the main graph as a guest (`userId = -1`).
